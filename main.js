...
;(($) => {
  var travelflatTheme = {
    ...
    events: function () {
      ...
      this.config.$document.on("ready", () => {
        // Retina Logos
        this.retinaLogo()
      })

      // Run on Window Load
      this.config.$window.on("load", () => {})
    },

    // Placeholder for retinaLogo function (if needed, otherwise remove)
    retinaLogo: () => {
      // Implement retina logo logic if you have high-res logos
      // For now, it's a placeholder as the original JS had it.
    },
  } // end

  // Start things up
  travelflatTheme.init()

  //  show logo home2
  $("#showlogo").prepend('<a href="index.html"><img id="theImg" src="assets/images/logo/logo2.png" /></a>')

  // Check if niceSelect is available before initializing
  if ($.fn.niceSelect) {
  ...
  if ($(".main-header li.dropdown2 ul").length) {
    ...
    $(".navigation li.dropdown2 > a").on("click", (e) => {
      e.preventDefault()
    })

    //Disable dropdown parent link
    $(".main-header .navigation li.dropdown2 > a,.hidden-bar .side-menu li.dropdown2 > a").on("click", (e) => {
      e.preventDefault()
    })

    $(".price-block .features .arrow").on("click", (e) => {
      $(e.target.offsetParent.offsetParent.offsetParent).toggleClass("active-show-hidden")
    })
  }
  // Mobile Nav Hide Show
  if ($(".mobile-menu").length) {
    //$('.mobile-menu .menu-box').mCustomScrollbar(); // Commented out as mCustomScrollbar is not included
    ...
    //Menu Toggle Btn
    $(".mobile-menu .menu-backdrop, .close-btn").on("click", () => {
    ...
    ajaxCall: (action) => {
      ...
      $.ajax({
        ...
        error: (jqXHR, textStatus, errorThrown) => {
          // alert('Connection error'); // Removed alert for better UX
          console.error("Subscription error:", textStatus, errorThrown)
          objUse.subscribeMsg.html(objUse.failure_message).fadeIn(500)
        },
        complete: (data) => {
